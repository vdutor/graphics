...
class Rasterizer {
 public:
  ...
  static bool Create(int width, int height,
                     ...
                     float clear_b = 0.0f, float clear_depth = 1.0f);
  ...
  bool SetUniformMatrix(const std::string& name, int num_columns, int num_rows,
                        bool transpose, absl::Span<const T> matrix);
...
template <typename T>
bool Rasterizer<T>::Create(int width, int height,
                           const std::string& vertex_shader_source,
                           const std::string& geometry_shader_source,
                           const std::string& fragment_shader_source,
                           std::unique_ptr<Rasterizer>* rasterizer,
...
bool Rasterizer<T>::Render(int num_points, absl::Span<T> result) {
  ...
  RETURN_FALSE_IF_GL_ERROR(glDisable(GL_BLEND));
  RETURN_FALSE_IF_GL_ERROR(glEnable(GL_DEPTH_TEST));
  RETURN_FALSE_IF_GL_ERROR(glDisable(GL_CULL_FACE));
  ...
  for (const auto& buffer : shader_storage_buffers_) {
    ...
    if (program_->GetResourceProperty(name, GL_SHADER_STORAGE_BLOCK, 1,
      ...
      continue;
    RETURN_FALSE_IF_ERROR(buffer.second->BindBufferBase(slot));
  }

  // Bind the program after the last call to SetUniform, since
  // SetUniform binds program 0.
  RETURN_FALSE_IF_ERROR(program_->Use());
  auto program_cleanup =
      tensorflow::gtl::MakeCleanup([]() { glUseProgram(0); });

  RETURN_FALSE_IF_ERROR(render_targets_->BindFramebuffer());
  RETURN_FALSE_IF_GL_ERROR(glViewport(0, 0, render_targets_->GetWidth(),
                                      render_targets_->GetHeight()));
  RETURN_FALSE_IF_GL_ERROR(glClearColor(clear_r_, clear_g_, clear_b_, 1.0));
  RETURN_FALSE_IF_GL_ERROR(glClearDepthf(clear_depth_));
  RETURN_FALSE_IF_GL_ERROR(glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT));

  RETURN_FALSE_IF_GL_ERROR(glDrawArrays(GL_POINTS, 0, num_points));
  RETURN_FALSE_IF_ERROR(render_targets_->ReadPixels(result));
...
bool Rasterizer<T>::SetShaderStorageBuffer(const std::string& name,
  ...
  RETURN_FALSE_IF_ERROR(shader_storage_buffers_.at(name)->Upload(data));
...
bool Rasterizer<T>::SetUniformMatrix(const std::string& name, int num_columns,
  ...
  RETURN_FALSE_IF_ERROR(program_->GetResourceProperty(
  ...
  RETURN_FALSE_IF_GL_ERROR(std::get<2>(type_info->second)(
