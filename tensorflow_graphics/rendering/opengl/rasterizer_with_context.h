...
RasterizerWithContext<T>::~RasterizerWithContext() {
  ...
  if (egl_context_->MakeCurrent() != tensorflow::Status::OK()) {
    std::cerr
...
tensorflow::Status RasterizerWithContext<T>::Create(
  ...
  RETURN_IF_TF_ERROR(EGLOffscreenContext::Create(&offscreen_context));
  RETURN_IF_TF_ERROR(offscreen_context->MakeCurrent());
  ...
  RETURN_IF_TF_ERROR(offscreen_context->Release());
...
tensorflow::Status RasterizerWithContext<T>::Render(int num_points,
                                                    absl::Span<T> result) {
  RETURN_IF_TF_ERROR(egl_context_->MakeCurrent());
  auto context_cleanup =
      MakeCleanup([this]() { this->egl_context_->Release(); });
  RETURN_IF_TF_ERROR(Rasterizer<T>::Render(num_points, result));
...
tensorflow::Status RasterizerWithContext<T>::SetShaderStorageBuffer(
  ...
  auto context_cleanup =
      MakeCleanup([this]() { this->egl_context_->Release(); });
  RETURN_IF_TF_ERROR(Rasterizer<T>::SetShaderStorageBuffer(name, data));
  // context_cleanup calls EGLOffscreenContext::Release here.
  return tensorflow::Status::OK();
}
...
tensorflow::Status RasterizerWithContext<T>::SetUniformMatrix(
  ...
  auto context_cleanup =
      MakeCleanup([this]() { this->egl_context_->Release(); });
  RETURN_IF_TF_ERROR(Rasterizer<T>::SetUniformMatrix(
